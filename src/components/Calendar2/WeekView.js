				...
				for (let i = 0; i < slot_count; ++i){
					...
					slots.push(
						<div 
							className={`
								myrow slot 
								${(this.props.interval === 5) ? "slot-sm" : "slot-md"} 
								${(i === slot_count - 1) ? "border-bot" : "border-bot-light"}
								${(!this.isAvailable(dateFns.parse(cloneTime))) ? "not-available" : "available"}
								${(this.isInMouseOverBlock(dateFns.parse(cloneTime))) ? "selectable" : ""}
							`} 
							key={i}
							// eslint-disable-next-line
							onMouseEnter={() => { return this.updateMouseOverBlock(cloneTime)}}
							onMouseLeave={this.clearMouseOverBlock}
